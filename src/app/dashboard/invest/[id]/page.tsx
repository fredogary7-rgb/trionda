...
export default function InvestDetailPage() {
  ...
  return (
    <div className="max-w-lg mx-auto p-4 sm:p-6 lg:p-8">
      <button onClick={() => router.push("/dashboard/invest")} className="text-gray-400 hover:text-white text-sm mb-4 flex items-center gap-1">
      ...
      <div className="glass-card p-6 mb-4">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-lg font-black text-white">{inv.plan.name}</h2>
        ...
        <div className="mb-6">
          ...
          <div className="w-full h-2 rounded-full bg-[#0A0F1E] overflow-hidden">
        ...
        <div className="grid grid-cols-2 gap-3 mb-6">
          <div className="bg-[#0A0F1E] rounded-xl p-3 text-center">
            ...
            <p className="text-lg font-black text-accent-400">{amount.toLocaleString("fr-FR")} F</p>
          </div>
          <div className="bg-[#0A0F1E] rounded-xl p-3 text-center">
            <p className="text-xs text-gray-500 mb-1">Gain/jour</p>
            <p className="text-lg font-black text-emerald-400">{dailyGain.toLocaleString("fr-FR")} F</p>
          </div>
          <div className="bg-[#0A0F1E] rounded-xl p-3 text-center">
            <p className="text-xs text-gray-500 mb-1">Gain cumulé</p>
        ...
        <div className="bg-[#0A0F1E] rounded-xl p-4 space-y-2 text-sm">
          <div className="flex justify-between"><span className="text-gray-500">Jours écoulés</span><span>{daysElapsed} / {daysTotal}</span></div>
          <div className="flex justify-between"><span className="text-gray-500">Gain total attendu</span><span className="text-gold-400 font-semibold">{(dailyGain * daysTotal).toLocaleString("fr-FR")} FCFA</span></div>
          <div className="flex justify-between"><span className="text-gray-500">Date de fin</span><span className="text-accent-400">{endDate.toLocaleDateString("fr-FR")}</span></div>
        </div>
      </div>
    </div>
  );
