    ...
    return (
      <div className="max-w-md mx-auto p-4 sm:p-6 lg:p-8 text-center">
        <div className="glass-card p-8">
          ...
          <p className="text-gray-400 text-sm mb-4">Votre demande de retrait de <span className="text-accent-400 font-bold">{parseInt(amount).toLocaleString("fr-FR")} FCFA</span> a été enregistrée.</p>
          <p className="text-xs text-gray-500 mb-6">Le traitement peut prendre 24-48h.</p>
          <button onClick={() => { setStep(1); setDone(false); setAmount(""); setPhone(""); setName(""); }}
            className="btn-primary text-sm">Nouveau retrait</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-md mx-auto p-4 sm:p-6 lg:p-8">
      ...
      <div className="flex items-start gap-2 p-3 bg-flame-500/5 border border-flame-500/10 rounded-xl mb-5">
        ...
        <p className="text-[11px] text-flame-300">Vérifiez bien le numéro et le montant avant de valider. Les retraits sont traités sous 24-48h.</p>
      </div>

      {msg.text && (
        <div className={`mb-4 p-3 rounded-xl text-xs ${msg.type === "error" ? "bg-flame-500/10 text-flame-400" : "bg-emerald-500/10 text-emerald-400"}`}>{msg.text}</div>
      )}
